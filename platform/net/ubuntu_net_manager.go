...
{{ if .DNSServers }}
prepend domain-name-servers {{ .DNSServers }};{{ end }}
`
...
func (net UbuntuNetManager) writeDHCPConfiguration(dnsServers []string, opts boshsys.ConvergeFileContentsOpts) (bool, error) {
	...
	type DHClientConfig struct {
		HostnameToSend string
		DNSServers     string
	}

	hostnameToSend := `"<hostname>"`
	if net.sendHostname {
		hostnameToSend = "= gethostname()"
	}

	err := t.Execute(buffer, DHClientConfig{
		HostnameToSend: hostnameToSend,
