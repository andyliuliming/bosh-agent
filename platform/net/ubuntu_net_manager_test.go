...
func describeUbuntuNetManager() {
	var (
		...
		netManagerSendHostname        UbuntuNetManager
	...
	BeforeEach(func() {
		...
		netManagerSendHostname = NewUbuntuNetManager(
			fs,
			cmdRunner,
			ipResolver,
			interfaceConfigurationCreator,
...
`))
		...
		It("write a dhcp configuration if send real hostname", func() {
			...
			err := netManagerSendHostname.SetupNetworking(boshsettings.Networks{"dhcp-network": dhcpNetworkWithoutDNS}, nil)
			Expect(err).ToNot(HaveOccurred())
